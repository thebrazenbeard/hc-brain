...
| Deep-memory archival/consolidation | ARCHITECTURALLY_REQUIRED + SPECIFIED | REFERENCE_IMPLEMENTED for explicit candidate/admission boundary only | DEMONSTRATED_INTEGRATED for admission gate; retrieval/consolidation incomplete | No | `deep memory storage/`; `runtime/cognitive_core/` |
...
| Cognition: perception, world model, counterfactuals, metacognition, relational reasoning | ARCHITECTURALLY_REQUIRED + SPECIFIED | REFERENCE_IMPLEMENTED only for observation-to-interpretation integration loop; world model/planning/metacognition still absent | DEMONSTRATED_INTEGRATED for bounded loop | No | `cognition/`; cognitive core |
