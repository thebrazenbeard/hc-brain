...
ALLOWED_PARTITION_WRITE_POLICIES = {
    "BLOCK",
    "BOUNDED_ISLAND",
    "MERGE_CANDIDATES_ONLY",
    "READ_ONLY",
}

ALLOWED_PARTITION_READ_POLICIES = {
    "CURRENT_ONLY",
    "LOCAL_ALLOWED",
    "STALE_WITH_LABEL",
    "READ_ONLY",
...
REVIEW_RECEIPT_REQUIRED_FIELDS = (
    "receipt_id",
    "subject_repo",
    "subject_head",
    "reviewed_scope",
...
def validate_state_family_profiles(document: dict[str, Any]) -> list[str]:
    ...
    for index, profile in enumerate(profiles):
        ...
        consistency_class = profile.get("consistency_class")
        if consistency_class not in ALLOWED_CONSISTENCY_CLASSES:
            errors.append(
                f"{label} unknown consistency_class: {consistency_class!r}"
            )
        ...
        if write_policy not in ALLOWED_PARTITION_WRITE_POLICIES:
            errors.append(
                f"{label}.partition_write_policy is missing or unknown"
            )

        read_policy = profile.get("partition_read_policy")
        if read_policy not in ALLOWED_PARTITION_READ_POLICIES:
            errors.append(
                f"{label}.partition_read_policy is missing or unknown"
            )

        merge_rule = profile.get("merge_or_reconciliation_rule")
        if not _is_nonempty_string(merge_rule):
            errors.append(
                f"{label}.merge_or_reconciliation_rule must be non-empty"
            )
        if (
            write_policy == "MERGE_CANDIDATES_ONLY"
            and not _is_nonempty_string(merge_rule)
        ):
            errors.append(
                f"{label} MERGE_CANDIDATES_ONLY requires reconciliation rule"
            )
        ...
        if (
            profile.get("protected") or profile.get("continuity_bearing")
        ) and not _is_nonempty_string(recovery_policy):
            errors.append(
                f"{label}.recovery_fence_policy required for "
                "protected/continuity-bearing family"
            )
    ...
    for path in (
        ...
        if path.is_file():
            ...
            if not _is_nonempty_string(document.get("schema_version")):
                errors.append(f"{path.relative_to(root)} missing schema_version")
            if not _is_nonempty_string(document.get("status")):
                errors.append(f"{path.relative_to(root)} missing status")
            if not _is_nonempty_string(document.get("scope")):
                errors.append(f"{path.relative_to(root)} missing scope")
